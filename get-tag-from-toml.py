...
def filter_workspace_members_by_ignored(workspace_members, ignored_members):
    return [member for member in workspace_members if x not in ignored_members]   
...
def get_a_new_tag(members_versions, current_version):
    ...
    for i in len(new_version_splitted):
        if int(current_version_splitted[i]) < int(new_version_splitted[i]):
            return members_versions[0]
    raise RuntimeError("A new version is equal or smaller that the current one")
...
def main():
    ...
    return get_a_new_tag(versions, current_version)
