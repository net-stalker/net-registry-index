...
# pylint: disable=import-error, wrong-import-position, unspecified-encoding, consider-using-with
import sys
import unittest
import os
...
class TestChangelogPreparator(unittest.TestCase):
    ...
    def create_changelog_backup(self):
        ...
        os.rename(
            os.path.join(CARGO_MANIFEST_DIR, 'CHANGELOG.md'),
            os.path.join(CARGO_MANIFEST_DIR, 'test.md')
        )
